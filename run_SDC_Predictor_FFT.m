function [U_val, V_val, X, Y, U_pred_out, V_pred_out, frame_time] = ...
    run_SDC_Predictor_FFT(inFile, snap_A, snap_B, H, W, U_pred_in, V_pred_in, static_mask)
% RUN_SDC_PREDICTOR_FFT  SDC with a temporal predictor whose warm start is
%   seeded by an FFT cross-correlation field.
%
%   On the first frame the predictor is computed with run_FFT (which shares
%   the same grid), giving a dense initial guess. Subsequent frames use the
%   previous validated field as predictor. In all cases the SDC core then
%   refines the displacement with a targeted search around the predictor.
%
% INPUTS:
...
%                          (empty [] triggers an FFT-seeded warm start).
...
%                          correlation. Also forwarded to the FFT warm start so
%                          both stages filter consistently. If omitted/empty,
...
%   frame_time               Processing time (FFT warm start + SDC + validation) [s].
    ...
    % --- 0. WARM-START CHECK + BOUNDARY EXTRAPOLATION ---
    is_warm_start = isempty(U_pred_in);

    % Boundary condition (spatial extrapolation): flow runs right-to-left,
    % so particles enter through the last column, which has no history.
    % Copy the velocity from the adjacent column.
    if ~is_warm_start && size(U_pred_in, 2) > 1
        U_pred_in(:, end) = U_pred_in(:, end-1);
        V_pred_in(:, end) = V_pred_in(:, end-1);
    end

    % First frame: seed the predictor with an FFT field.
    if is_warm_start
        disp('  -> Initial frame: computing FFT predictor...');
        [U_fft, V_fft, ~, ~, time_fft] = run_FFT(inFile, snap_A, snap_B, H, W, static_mask);
    else
        time_fft = 0;
    end
    ...
    % Window size: fixed on the warm start, then propagated to keep the grid
    ...
    % Window-center grid
    x_lin = (0:num_win_x-1) * StepSize + 1 + (WinSize-1)/2;
    y_lin = (0:num_win_y-1) * StepSize + 1 + (WinSize-1)/2;
    [X, Y] = meshgrid(x_lin, y_lin);

    % Predictor source: FFT field on the warm start, previous field otherwise
    if is_warm_start
        U_pred_sdc = U_fft;     % FFT and SDC share the same grid
        V_pred_sdc = V_fft;
    else
        U_pred_sdc = U_pred_in;
        V_pred_sdc = V_pred_in;
    end

    % SDC correlation core (targeted search around the predictor)
    for wy = 1:num_win_y
        for wx = 1:num_win_x
            ...
            % Predictor logic: targeted search or bounded fallback if predictor is NaN
            if isnan(U_pred_sdc(wy, wx)) || isnan(V_pred_sdc(wy, wx))
                % Predictor unavailable: bounded full search to recover the window
                r      = min(WarmStartRadius, WinSize-1);
                lags_x = -r : r;
                lags_y = -r : r;
            else
                u_pred = round(U_pred_sdc(wy, wx));
                v_pred = round(V_pred_sdc(wy, wx));
            ...
            if max_val >= min_corr_peak
                ...
                if row_peak > 1 && row_peak < size(corr_map, 1) && ...
                   col_peak > 1 && col_peak < size(corr_map, 2)
                    C0_x = corr_map(row_peak, col_peak); Cm_x = corr_map(row_peak, col_peak - 1); Cp_x = corr_map(row_peak, col_peak + 1);
                    C0_y = corr_map(row_peak, col_peak); Cm_y = corr_map(row_peak - 1, col_peak); Cp_y = corr_map(row_peak + 1, col_peak);
                    if Cm_x > 0 && Cp_x > 0 && C0_x > 0
                        dx = (log(Cm_x) - log(Cp_x)) / (2 * (log(Cm_x) - 2*log(C0_x) + log(Cp_x)));
                    end
                    if Cm_y > 0 && Cp_y > 0 && C0_y > 0
                        dy = (log(Cm_y) - log(Cp_y)) / (2 * (log(Cm_y) - 2*log(C0_y) + log(Cp_y)));
                    end
                end
    ...
    % --- 5. PROCESSING TIME ---
    % Includes the FFT warm start (frame 1 only). Event-read time excluded:
    % it depends heavily on system load.
    frame_time = time_fft + time_sdc + time_val;
end
