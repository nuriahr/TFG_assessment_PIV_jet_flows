function [U_val, V_val, X, Y, frame_time] = run_SDC(inFile, snap_A, snap_B, H, W, static_mask)
% RUN_SDC  Sparse Direct Correlation (full search, no predictor) from .hdf5.
%
%   Correlation is computed by iterating only over active events inside each
%   interrogation window (sparse approach), instead of over every pixel as
%   in DCC. This is much faster when the event images are sparse.
%
% INPUTS:
...
%                    omitted/empty, no filtering is applied.
%
% OUTPUTS:
...
%   frame_time     Processing time (correlation + validation) [s].
    ...
    if snap_B + Naccum > numel(trigOn)
        error('Requested frames exceed the available time/triggers in the file.');
    end
    ...
    % --- 3. SPARSE DIRECT CORRELATION (FULL SEARCH) ---
    tic;
    ...
    for wy = 1:num_win_y
        for wx = 1:num_win_x
            ...
            % Active-event coordinates inside window1 (sparse core)
            [p_r, p_c] = find(window1);
            num_p = numel(p_r);
            if num_p < min_particles_threshold, continue; end

            % Correlation map: iterate over events only, not pixels
