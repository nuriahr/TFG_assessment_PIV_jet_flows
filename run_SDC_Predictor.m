function [U_val, V_val, X, Y, U_pred_out, V_pred_out, frame_time] = ...
    run_SDC_Predictor(inFile, snap_A, snap_B, H, W, U_pred_in, V_pred_in, static_mask)
% RUN_SDC_PREDICTOR  Sparse Direct Correlation with a temporal predictor.
%
%   Uses the previous frame's velocity field to perform a targeted search
%   around the predicted displacement, drastically reducing the correlation
%   cost after the first (warm-start) frame.
%
% INPUTS:
...
%                          (empty [] triggers a full-search warm start).
...
%   frame_time               Processing time (correlation + validation) [s].
    ...
    % --- 3. SPARSE DIRECT CORRELATION + TEMPORAL PREDICTOR ---
    tic_sdc = tic;
    ...
    % First frame? -> full-search warm start
    ...
    % Window size: computed once on the warm start, then propagated
    ...
    for wy = 1:num_win_y
        for wx = 1:num_win_x
            ...
            % --- Predictor logic: bounded full search range (warm start) or targeted ---
            if is_warm_start || isnan(U_pred_in(wy, wx)) || isnan(V_pred_in(wy, wx))
